    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for i, s in enumerate(splits):
            ...
            if len(part) > self.chunk_size:
                good_splits.extend(self._split(part, new_separators))
            else:
        ...
        for s in good_splits:
            if not current_chunk:
                current_chunk = s
            elif len(current_chunk) + len(s) <= self.chunk_size:
                current_chunk += s
            else:
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        return {
            ...
            "by_sentences": {
                "count": len(sentences),
                "avg_length": sum(len(c) for c in sentences) / len(sentences) if sentences else 0,
                "chunks": sentences
            },
            "recursive": {
                "count": len(recursive),
